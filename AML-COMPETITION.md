...
| 组件 | 许可证 | 用途 | 我方改动 |
|---|---|---|---|
| memorycore-aml（参赛仓库, 基于 origin-memorycore） | MIT | 治理层（冷热路由/去重/溢流/回收站/衰减） | 新增 AML HTTP 适配层 `memorycore/aml_server.py`；`cold_store_client.py` 增加 author_id 等身份参数透传（默认 None，原行为不变）；`stats()` 增加 all_sessions 计数；依赖增加 uvicorn |
...
- 作者：moonandecho
- 主仓库: https://github.com/moonandecho/origin-memorycore
- 参赛仓库: https://github.com/moonandecho/memorycore-aml
